...
public sealed class XlsxExportService : IExportService
{
    ...
    private static void WriteDetail(XLWorkbook wb, List<CalendarEvent> events)
    {
        ...
        {
            ws.Cell(row, 1).Value = e.Category ?? string.Empty;
            ws.Cell(row, 2).Value = e.Client ?? string.Empty;
            ws.Cell(row, 3).Value = e.Project ?? string.Empty;
            ws.Cell(row, 4).Value = e.Topic ?? e.Subject;
            ws.Cell(row, 5).Value = Math.Round(e.DurationHours, 2);
